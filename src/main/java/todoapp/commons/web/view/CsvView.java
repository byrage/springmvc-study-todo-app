...
public class CsvView extends AbstractView implements SpreadsheetSupport {
    ...
    protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
        ...
        if (sheet.hasHeader()) {
            String header = sheet.getHeader().map(row -> joining(row.getCells().stream().map(cell -> cell.getValue()).collect(Collectors.toList()), ",")).orElse("");
            response.getWriter().println(header);
        }

        if (sheet.hasRows()) {
            for (Spreadsheet.Row row : sheet.getRows()) {
                String line = joining(row.getCells().stream().map(cell -> cell.getValue()).collect(Collectors.toList()), ",");
