...
public interface SpreadsheetSupport {
    ...
    default Spreadsheet obtainSpreadsheet(Map<String, Object> model) {
        List<Spreadsheet> spreadsheets = model.values().stream()
    ...
    /**
     * 주어진 반복자에 값을 연결해서 하나의 문자열로 만듭니다.
     ...
     * @return
     */
    default String joining(Iterable<?> iterable, CharSequence delimiter) {
        return StreamUtils.createStreamFromIterator(iterable.iterator())
                          .map(String::valueOf)
